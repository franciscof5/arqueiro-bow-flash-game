...
class arqueiro extends MovieClip {
	...
	*/
	...
	private function colisao():Void {
		// cria um array com todos os objetos de colisão e da um for para testar todos
		//essa função exige muito do processador, tomar cuidado para não colocar muitos objetos intrasponiveis no palco
		//for(var i=0; i<_root.objetosIntrasponiveis_array.length; i++) {
			/*if(this.hitTest(_root.lago1) or this.hitTest(_root.lago2) or this.hitTest(_root.bloco1) or this.hitTest(_root.bloco2))
			multiplicadorColisao = -0.001; //rebate o jogador para trás muito devagar suavemente
			else
			multiplicadorColisao = 1;*/
		//}
	...
	private function falar (string:String, deOndeVeioAfala:TextField) {
		...
		//balaoNome.setPosicao(this._x, this._y);
	}
